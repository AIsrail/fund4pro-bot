...
2. Данные, а не предположения — каждое утверждение о проблеме должно иметь
   ...
   любую другую мета-пометку в тексте заявки — впиши осторожную оценку по
   близким данным ("по оценке, порядка...") или плановую величину, выведенную
   из бюджета (см. правило недостающих данных ниже); XYZ — редкое исключение.
...
БЮДЖЕТ ВСЕГДА ДЕТАЛИЗИРОВАННЫЙ, НИКОГДА одной обобщённой строкой на
категорию (прямое требование владельца, 26.09.2026: "бюджет надо всегда
писать детально... никогда не обобщайте по категориям"). Одна строка
"Проектные расходы — $25 000" — недостаточно, даже если донор дал под
категорию всего одну ячейку/строку в форме: раскладывай на конкретные
позиции с их ценами ("Обучение 25-30 фермеров — $Х", "Дегустационные туры —
$Y", "Транспорт для мероприятий — $Z", ...), которые в сумме дают итог
категории. Если форма донора физически даёт под категорию НЕСКОЛЬКО строк
(построчный бюджет с пустыми продолжениями после названия категории) —
заполняй КАЖДУЮ доступную строку конкретной позицией, а не одну и не
пустыми. Правило "не выдумывай данные" здесь не про отказ от детализации —
конкретные, разумно оценённые позиции (по рыночным ценам, как и остальные
плановые величины) полезнее и достовернее одной округлённой суммы на всю
категорию.
...
PLACEHOLDER_RULE = """
ПРАВИЛО НЕДОСТАЮЩИХ ДАННЫХ (критично, применяется ВЕЗДЕ — в черновике, в
финальной версии, при заполнении формы донора). Пользователю нужна ГОТОВАЯ
заявка: не менее 90% чисел и фактов в тексте должны быть конкретными, и не
более 10% — маркером XYZ. Порядок действий для каждой нужной цифры/факта:

1. Взять из собранной информации, из найденных в сети данных (со ссылкой на
   источник в тексте, если это статистика) или из ответов пользователя.
2. Если это ПЛАНОВАЯ величина проекта (число участников/благополучателей,
   количество мероприятий, единиц оборудования, охват, индикаторы, сроки,
   доли, стоимость единицы) — ВЫВЕДИ её сам из бюджета гранта и логики
   проекта (обратный ход «3 деревьев»: деньги -> количество -> результат) и
   типичных рыночных цен/нормативов. Пиши конкретное число с пометкой
   приблизительности: "около 120", "~35%", "порядка 2 400 USD". Плановые
   значения — это обязательство заявителя, их можно и нужно предложить.
3. Если это ВНЕШНЯЯ статистика (население, турпоток, распространённость
   проблемы), а найти её не удалось — дай осторожную оценку по близким
   данным (соседний регион, страна в целом, отраслевой норматив) с
   формулировкой "по оценке", "порядка", "около", БЕЗ выдуманной точности и
   без ссылки на несуществующий источник.
4. XYZ ставь ТОЛЬКО там, где число/факт нельзя ни найти, ни разумно оценить:
   ФИО конкретных людей, даты подписания, регистрационные/банковские
   реквизиты, названия конкретных партнёров, которых пользователь не называл.

Запрещено: писать мета-комментарии ("оценка, требует проверки", "нужны
точные данные", "уточнить у...") и громоздкие [XYZ: пояснение] в тексте —
они выглядят как черновик. Оценка пишется как обычная часть предложения.
XYZ — короткий маркер прямо на месте значения, без скобок и пояснений.

Пример (неправильно): "XYZ туристов в год, рост на XYZ%, XYZ тонн мусора".
Пример (правильно): "по оценке, порядка 40 000 туристов в год, рост около
10% ежегодно; на маршрутах накапливается около 25 тонн мусора за сезон".
""".strip()
...
async def label_donor_form(filename: str, form_text: str) -> str:
    ...
    system_prompt = (
        ...
        "по названию/структуре. КЛЮЧЕВОЕ РАЗЛИЧИЕ, на которое нужно явно "
        "указать в метке: это ФОРМА С ПОЛЯМИ ДЛЯ ЗАПОЛНЕНИЯ (таблица/бланк "
        "с местами под короткие ответы — название организации, даты, "
        "суммы) или ГАЙДЛАЙНЫ/ИНСТРУКЦИЯ ПО СОДЕРЖАНИЮ (заголовки разделов "
        "с описанием, ЧТО написать в каждом — 'Резюме проекта', 'История "
        "проекта', 'Задачи' и т.п. — без полей для заполнения; сама заявка "
        "по такому документу пишется отдельным свободным текстом). "
        "Примеры меток: 'Форма профиля организации (поля)', 'Форма заявки "
        "— технические поля', 'Гайдлайны содержания заявки (не форма)', "
        "'Бюджетная форма Excel (поля)', 'Итоговый отчёт по гранту (поля)'. "
        "Без кавычек вокруг всей метки и точки в конце."
    )
...
async def donor_perspective_review(document_text: str, donor_info: str) -> str:
    """"Собеседник-эксперт", который смотрит на готовый документ глазами
    донора — отдельный, ИЗОЛИРОВАННЫЙ вызов БЕЗ истории разговора, в котором
    документ создавался. Основано на приёме "fresh-eyes reviewer" (Pass 2 —
    вопросы рецензента) из скилла grant-writing: тот, кто писал документ,
    помнит контекст беседы и поэтому не видит пробелов, которые сразу
    заметит читатель без этой памяти — a) вопросы донора, на которые текст
    не отвечает, и b) факты/цифры, поданные как данность без источника
    в самом тексте. Это ДРУГАЯ проверка, чем red-flags-аудит (self-review
    по чек-листу) — здесь модель намеренно не видит, как строился документ.

    Best-effort и не блокирует отправку: при любой ошибке возвращает пустую
    строку, вызывающий код просто не покажет секцию обзора."""
    if not document_text.strip():
        return ""
    system_prompt = (
        "Ты — рецензент донора/инвестора, который видит этот документ ВПЕРВЫЕ "
        "и не знает, как и почему он написан именно так. Прочитай его как "
        "реальный эксперт, оценивающий заявку на финансирование: задай себе "
        "5-8 вопросов, которые донор реально задал бы по приоритетам "
        "конкурса (если они названы) и по слабым местам самого текста, и "
        "ответь на каждый ИСКЛЮЧИТЕЛЬНО по тому, что написано в документе — "
        "если текст не отвечает на вопрос, прямо скажи, что не отвечает, не "
        "додумывай за автора. Отдельно перечисли числа/факты/проценты, "
        "поданные как данность, но без указания источника в самом тексте.\n\n"
        "Ответь КОРОТКИМ списком (не длиннее 5-7 пунктов) — только то, что "
        "реально нашёл, без общих оценочных фраз вроде 'в целом неплохо'. "
        "Если серьёзных пробелов нет — так и скажи одной строкой."
    )
    user_message = f"Донор/конкурс: {donor_info.strip() or 'не указан'}\n\nДокумент:\n{document_text[:12000]}"
    try:
        return (await call_claude(system_prompt, user_message, max_tokens=800)).strip()
    except Exception as exc:
        logger.warning("donor_perspective_review failed: %s: %s", type(exc).__name__, exc)
        return ""
...
async def generate_final_document(session_data: dict) -> str:
    ...
    no_questions_rule = (
        ...
        "нет в контексте — примени правило недостающих данных выше "
        "(оценка от бюджета/близких данных, XYZ не более 10%) и продолжай заполнение дальше, а не "
...
async def fill_table_field(question: str, columns: list[str], session_data: dict) -> list[list[str]]:
    ...
    system_prompt = (
        ...
        f"ход «3 деревьев» из методологии). РЕАЛЬНЫЙ ИНЦИДЕНТ: итог вида "
        f"ровно 5000 — для донора красный флаг «заявитель не считал, подогнал "
        f"под лимит». Считай построчно от реальной цены/количества (единица "
        f"товара/услуги на местном рынке, не аккуратный процент от лимита) — "
        f"так итог естественно получится НЕ круглым (…5140, …5250, а не "
        f"…5000/…5500). Не выводи административные/M&E/контингенси-строки "
        f"РОВНО как круглый процент от лимита (не «ровно 12%») — посчитай их "
        f"от уже посчитанных прямых расходов, тогда и они не будут круглыми. "
        f"Никогда не выдумывай "
...
async def resolve_placeholders_batch(items: list[dict], session_data: dict) -> dict[str, str]:
    """Второй проход после заполнения формы: если доля XYZ превысила 10%,
    просим модель переписать проблемные фрагменты, заменив XYZ конкретными
    оценками, выведенными из бюджета гранта и контекста проекта (по правилу
    недостающих данных из системного промпта). items — [{"id","text"}].
    Возвращает {id: новый_текст} только для успешно обработанных фрагментов."""
    if not items:
        return {}
    listing = "\n".join(f'{it["id"]}: {it["text"]}' for it in items)
    system_prompt = (
        f"{SYSTEM_PROMPT}\n\nНиже фрагменты заявки, в которых остались маркеры "
        f"XYZ. Перепиши КАЖДЫЙ фрагмент, заменив XYZ конкретными значениями: "
        f"плановые величины (охват, количество, индикаторы, сроки, суммы) выведи "
        f"из бюджета гранта и логики проекта, внешнюю статистику оцени осторожно "
        f"по близким данным. Пиши приблизительные значения с пометкой "
        f"\"около\"/\"~\". Оставь XYZ ТОЛЬКО для ФИО, дат подписи, реквизитов и "
        f"названий конкретных партнёров. Остальной текст фрагмента не меняй. "
        f"Без мета-комментариев.\n\nКонтекст проекта:\n{_session_summary(session_data)}\n\n"
        f'Верни СТРОГО валидный JSON без markdown-обрамления: {{"id": "новый текст", ...}}.'
    )
    try:
        raw = await call_claude(system_prompt, listing, max_tokens=4000, prefer_anthropic=True)
        start, end = raw.find("{"), raw.rfind("}")
        parsed = json.loads(raw[start:end + 1])
    except Exception as exc:
        logger.warning("resolve_placeholders_batch failed: %s: %s", type(exc).__name__, exc)
        return {}
    return {
        str(k): str(v).strip()
...
async def fill_form_fields_batch(
    ...
    system_prompt = (
        ...
        f"ВАЖНО:\n"
        f"- Правило недостающих данных из системного промпта: не менее 90% "
        f"чисел/фактов конкретные — плановые величины (охват, количество, "
        f"индикаторы, сроки) выводи из бюджета гранта и пиши с пометкой "
        f"\"около\"/\"~\"; внешнюю статистику оценивай осторожно по близким "
        f"данным; XYZ — только для ФИО, дат подписи, реквизитов.\n"
        f"- Голого \"XYZ\" не бывает — заглушка обязана нести единицу измерения "
        f"и смысл, чтобы заказчик видел, ЧТО именно вписать, а не догадывался: "
        f"не \"XYZ\", а \"XYZ территорий очищено\", \"XYZ — годовой бюджет "
        f"организации за прошлый год, в долларах США\". Несколько XYZ в одной "
        f"строке — нормально, если показывают структуру показателя (\"XYZ "
        f"обращений подано, из них XYZ рассмотрено\").\n"
        f"- Если вопрос формы перечисляет несколько ожидаемых элементов ответа "
        f"по порядку (например \"опишите цель, задачи, деятельность и "
        f"ожидаемый результат\") — отвечай СТРОГО в этом же порядке, короткими "
        f"подзаголовками на каждый элемент. Донор читает ответ в порядке "
        f"вопроса; обратный порядок читается как неструктурированность.\n"
