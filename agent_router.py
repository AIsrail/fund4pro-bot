...
async def _send_docx(message: Message, text: str, session: dict) -> None:
    ...
    # РЕАЛЬНЫЙ ИНЦИДЕНТ: разделы "План действий"/"Бюджет проекта" несколько
    # раз подряд пропадали из готовой заявки без единого слова об этом —
    # docx_template_fill заполняет файл ПОСЛЕ того, как реплика модели уже
    # сформирована в run_agent_turn, так что сама модель не может отреагировать
    # на то, что узнаётся только на этом, более позднем шаге. Поэтому
    # предупреждение — не сгенерированный моделью текст, а прямое сообщение
    # от кода, гарантированно доходящее до пользователя, если что-то из
    # ключевых разделов не удалось перенести в форму донора с уверенностью.
    # ВАЖНО: только если файл реально успешно ушёл — иначе выше уже показано
    # сообщение об ошибке экспорта, и это предупреждение будет вводящим в
    # заблуждение ("файл отправлен выше", хотя он не отправлен).
    if not sent_ok:
        session.pop("_unfilled_donor_sections", None)
        return

    unfilled = session.pop("_unfilled_donor_sections", None)
    if unfilled:
        warning_lines = "\n".join(f"⚠️ {line}" for line in unfilled)
        await message.answer(
            "Файл собран и отправлен выше, но не всё удалось перенести в форму "
            "донора с полной уверенностью:\n\n"
            f"{warning_lines}\n\n"
            "Если пришлёте точные данные (конкретные мероприятия по задачам "
            "и/или статьи бюджета с суммами), я обновлю документ."
        )
