...
import config
from agent_playbook_modules import get_module
...
async def _execute_tool(name: str, args: dict, session: dict) -> str:
    ...
    if name == "load_playbook_module":
        topic = str(args.get("topic", "")).strip()
        loaded = session.setdefault("loaded_modules", [])
        if topic in loaded:
            return (
                f"Модуль '{topic}' уже был загружен раньше в этом разговоре "
                f"(смотри выше в истории) — используй то, что уже есть, не "
                f"нужно грузить повторно."
            )
        content = get_module(topic)
        if content is None:
            return f"Неизвестный модуль '{topic}'. Доступные модули перечислены в описании инструмента."
        loaded.append(topic)
        return content
...
async def run_agent_turn(session: dict, user_text: str) -> AgentTurnResult:
    ...
    loaded_modules = session.get("loaded_modules", [])
    system_prompt = build_system_prompt(project_data, flow, ui_language, doc_language, loaded_modules)
