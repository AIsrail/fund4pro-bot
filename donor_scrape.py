...
MAX_FILES_TO_FETCH = 6  # не заливать LLM-контекст десятками найденных ссылок
# РЕАЛЬНЫЙ ИНЦИДЕНТ: было 3 — у NED на странице конкурса ровно 4 обязательных
# файла (форма профиля, форма заявки, гайдлайны с содержательными вопросами,
# Excel-шаблон бюджета), и четвёртый (сам бюджетный шаблон) молча никогда не
# скачивался — ссылки в HTML идут именно в этом порядке, обрезка списком
# [:MAX_FILES_TO_FETCH] откусывала его. 6 — с запасом на донора с ещё большим
# пакетом документов, не раздувая контекст до отказа модели отвечать.
