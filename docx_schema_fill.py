...
def extract_template_schema(doc: "docx.Document") -> list[FieldSpec]:
    ...
    for table in doc.tables:
        # РЕАЛЬНЫЙ ИНЦИДЕНТ (живой тест FAO, 26.09.2026): таблица с ГРИД-
        # структурой (строка заголовка именует каждую колонку — "№ |
        # Мероприятие | Сроки | Ответственный", "Риски | Вероятность | Меры",
        # бюджетная "Статьи | Ед. | Стоимость | Количество | ВСЕГО | ...")
        # раньше получала РОВНО ОДНО поле на строку данных — метку в col[0]
        # (или первый непустой сосед) в пару с ПЕРВОЙ же соседней пустой
        # ячейкой. Остальные пустые ячейки той же строки (col[2], [3]...)
        # НИКОГДА не становились полями вообще — не на слабом провайдере, а
        # вообще ни на каком, потому что вопрос для них просто не
        # существовал в схеме. Итог у пользователя: план мероприятий
        ...
        for row_idx, row in enumerate(table.rows):
            ...
            if is_grid and len(unique_cells) >= 2:
                ...
                # непустому тексту — старая "label -> соседняя пустая
                # ячейка" логика при повторной обработке частично
